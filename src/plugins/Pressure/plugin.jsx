...
class Pressure extends React.Component {
  ...
  componentWillMount() {
    const isPressurePhone =
      pressureUtils.getType(this.getTargetList()) ===
      pressureUtils.PRESSURE_TYPE_PHONE;
    const hasCounter = !!this.props.widget.settings.count_text;
    if (hasCounter && isPressurePhone) {
      this.props
        .countTwilioCallsByWidget({ widgetId: this.props.widget.id })
  ...
  handleSubmit(data) {
    if (data.pressureType === pressureUtils.PRESSURE_TYPE_EMAIL) {
      ...
      if (
        ...
        this.state.selectableTargetList
      ) {
        this.setState({
        ...
        this.props.twilioCall(
          {
            ...
            from: data.phone,
            to: this.getEmailTarget(
              arrayUtils.shuffle(this.getTargetList())[0]
            ),
          },
          true
        );
  ...
  render() {
    ...
    return finishPressure ? (
      ...
      <div className="pressure-widget">
        ...
        <Targets targets={this.getTargetList() || []} />
        <Form
